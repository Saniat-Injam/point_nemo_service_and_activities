...
class BusinessOwnerHomeController extends GetxController {
  ...
  String get userName {
    if (Get.isRegistered<CommonProfileController>()) {
      ...
      return name.isEmpty ? 'Business Owner' : name;
    }
  ...
  List<double> get thisWeekPerformance {
    ...
    final thisWeek = performance.length >= 7 
        ...
        : performance;
    final maxCount = performance.fold<int>(1, (prev, e) => e.count > prev ? e.count : prev); // Use 1 as min to avoid division by zero
  ...
  Future<void> fetchDashboardData() async {
    ...
    final response = await _networkCaller.getRequest(AppUrls.ownerDashboard);

    if (response.isSuccess) {
      final dashboardData = BusinessOwnerDashboardModel.fromJson(response.responseData['data'] ?? {});
      
      rawTotalEarnings.value = dashboardData.totalEarnings;
      totalEarnings.value = '\$${_formatEarnings(dashboardData.totalEarnings)}';
      confirmedCount.value = dashboardData.confirmedCount;
